...
RUN_COPY_CMD="ndncon_autotest_copy_log-latFluc.sh"
...
function shapeNetwork()
{
	...
	log "shaping network: latency ${lat}ms, latency jitter +- ${lat_dev}ms, bandwidth ${bw}Kbit/s"
	SHAPE_NW_CMD1="sudo tc qdisc add dev ${TEST_IFACE1} root netem delay ${lat} ${lat_dev} " # default distribution is uniform
	# https://www.excentis.com/blog/use-linux-traffic-control-impairment-node-test-environment-part-2
	runCmd "${SHAPE_NW_CMD1}"
	SHAPE_NW_CMD0="sudo tc qdisc add dev ${TEST_IFACE0} root netem delay ${lat} ${lat_dev}"
...
function runtests()
{
	...
	i=1
	while read lat lat_dev bw type; do 
		log "running test $i (LATENCY: $lat LATENCY JITTER: $lat_dev BW: $bw TYPE: $type)"
		shapeNetwork $lat $lat_dev $bw
		testName="${i}-${type}-lat${lat}jitter${lat_dev}bw${bw}"
