...
def printCurrentBufferState():
	...
	prevFrame = None
	#print "loop in"
	for frame in currentBufferState.frames:
		if prevFrame:
			#print "in prevFrame"
			if prevFrame.frameType == frame.frameType and frame.frameType == 'D':
				seqNoDiff = frame.seqNo - prevFrame.seqNo
				if seqNoDiff > 1:
					print "seqno diff is greater than one in state with ts "+\
					str(currentBufferState.timestamp)+" frames "+str(prevFrame.seqNo)+" and "+str(frame.seqNo)
				#else:
					#print "ok"
		prevFrame=frame;
		#print "end this buffer"
