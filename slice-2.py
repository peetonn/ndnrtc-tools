...
# slices consumer log into smaller files per runs with specified statistics (with timestamp-2)
# example use: slice-2.py -f consumer-test2-camera.log -k "new state,buf est,buf tar,buf play,rtt est,D arr,lambda d" -v
...
def printKeywordDataBlock(kw,timestamp):
  global output
...
def onKeywordFound(timestamp, match, userData):
  ...
  printKeywordDataBlock(kwBlock,timestamp)
  return True
