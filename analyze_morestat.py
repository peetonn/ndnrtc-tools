...
class StatKeyword(Enum):
  ...
  rtx = 12
  skipNoKey =13
  skippedIncompleteDel =14
  skippedIncompleteKey = 15
  skippedBadGop = 16
  acquiredNum =17
  ...
  def __str__(self):
    ...
    StatKeyword.skipNoKey:'delta gop mismatch', StatKeyword.skippedIncompleteDel:'delta incomplete',\
    StatKeyword.skippedIncompleteKey:'key incomplete', StatKeyword.skippedBadGop:'invalid gop', StatKeyword.acquiredNum:'unlock'}[self]
...
NottoCalculateAverage = [statEntryRegex(StatKeyword.rescued),statEntryRegex(StatKeyword.recovered),statEntryRegex(StatKeyword.acquiredNum),statEntryRegex(StatKeyword.skipNoKey),\
statEntryRegex(StatKeyword.skippedIncompleteDel), statEntryRegex(StatKeyword.skippedIncompleteKey), statEntryRegex(StatKeyword.skippedBadGop),statEntryRegex(StatKeyword.rtx)]

statQoEEntryRegexString = '(?P<stat_entry>'+statEntryRegex(StatKeyword.skipNoKey)+'|'+statEntryRegex(StatKeyword.acquiredNum)+'|'+\
  statEntryRegex(StatKeyword.skippedIncompleteKey)+'|'+statEntryRegex(StatKeyword.skippedIncompleteDel)+'|'+statEntryRegex(StatKeyword.skippedBadGop)+')'
# statQoEEntryRegexString = '(?P<stat_entry>'+statEntryRegex(StatKeyword.acquiredNum)+')'
statQoEEntryRegex = re.compile(statQoEEntryRegexString)

def closeStatBlock(timestamp):
  global statBlock, runBlock, interestNo, dataNo, runNo, statBlockNum, NottoCalculateAverage
  # write header
  ...
  for key in statBlock.keys():
    ...
    if key in NottoCalculateAverage:
      mean = values  if values > 0 else 0
    else:
...
def closeRun(timestamp):
  ...
  global runNo, runBlock, lastTimestamp, runStartTime, chaseTime, NottoCalculateAverage, FrameSum, nacquiredNum
  ...
  if summaryFile:
    with open(summaryFile, "a") as f:
      ...
      for key in runBlock.keys():
        values = runBlock[key]
        if key in NottoCalculateAverage:
          # print('key: '+str(key) +', mean: '+str(values)+'\n')
          mean = values[-1] if len(values) > 0 else 0#not to calculate average
        else:
          mean = sum(values, 0.0) / len(values) if len(values) > 0 else 0
...
def onStatEntry(timestamp, match, userData):
  ...
  for m in statRegex.finditer(match.group('message')):
    ...
    if statEntry in NottoCalculateAverage:
      value = int(m.group('value'))
    else:
      value = float(m.group('value'))
    if not statEntry in statBlock.keys():
      print str(statEntry) + ' is not in stat block: '+str(statBlock)
    elif statEntry in NottoCalculateAverage:
      statBlock[statEntry]=statBlock[statEntry]+1 # count one time for this keyword
    else:
      statBlock[statEntry].append(value)
  return True

def onstatQoEEntry(timestamp, match, userData):
  global statBlock, lastTimestamp
  for m in statQoEEntryRegex.finditer(match.group('message')):
    statEntry = m.group('stat_entry')
    if not statEntry in statBlock.keys():
      print str(statEntry) + ' is not in stat block: '+str(statBlock)
    elif statEntry in NottoCalculateAverage:
...
if __name__ == '__main__':
  ...
  (str(StatKeyword.recovered), []), (str(StatKeyword.rescued), []),\
  (str(StatKeyword.rtx), []), (str(StatKeyword.skipNoKey), []), (str(StatKeyword.skippedIncompleteDel), []),\
  (str(StatKeyword.skippedIncompleteKey), []), (str(StatKeyword.skippedBadGop), []),\
  (str(StatKeyword.acquiredNum), [])])
  ...
  (str(StatKeyword.bufPlayable),[]), (str(StatKeyword.rttEst),[]), (str(StatKeyword.rttPrime),[]), (str(StatKeyword.lambdaD),[]), (str(StatKeyword.lambdaC),[]),\
  (str(StatKeyword.recovered), 0), (str(StatKeyword.rescued), 0),\
  (str(StatKeyword.rtx), 0), (str(StatKeyword.skipNoKey), 0), (str(StatKeyword.skippedIncompleteDel), 0),\
  (str(StatKeyword.skippedIncompleteKey), 0), (str(StatKeyword.skippedBadGop), 0),\
  (str(StatKeyword.acquiredNum), 0)])
  ...
  # statQoEEntryRegexString = '(?P<stat_entry>lock\s\[)'
  statQoEEntryActions = {}
  statQoEEntryActions['pattern'] = ndnlog.compileNdnLogPattern('.*', '.*', statQoEEntryRegexString)
  statQoEEntryActions['tfunc'] = timeFunc
  statQoEEntryActions['func'] = onstatQoEEntry

  if summaryFile:
    with open(summaryFile, "w") as f:
      f.write("***"+logFile+"\n")
  ndnlog.parseLog(logFile, [statQoEEntryActions, statEntryActions, chaseTrackActions, interestExpressActions, dataReceivedActions, rebufferingActions])
