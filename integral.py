...
if len(sys.argv) < 2:
  ...
  print ""
  print "Calculates integral (histogram) of a selected column in a CSV file. If no column is specified, "
			...
			if header == None: # intialize, assuming first line is a header
				...
				if x1-x0 > interval:
					...
					for k in t.keys():
						if k != 'timestamp':
							sY = sum(window[k][0:-1])
							comps[header.index(k)] = str(sY)
							window[k] = [t[k]]
					window['timestamp'] = [x1*1000]
					for c in comps:
						sys.stdout.write(c)
